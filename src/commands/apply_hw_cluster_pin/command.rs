...
pub async fn exec(
    ...
    let target_hsm_group_member_vec: Vec<String> = hsm::group::utils::get_member_vec_from_hsm_name_vec(
        shasta_token,
        shasta_base_url,
        shasta_root_cert,
    ...
    for (hw_component, qty) in &user_defined_target_hsm_hw_component_count_hashmap {
            ...
            .is_some_and(|value| value >= qty)
        {
            ...
            return Err(Error::Message("There are not enough resources to fulfill user request.".to_string()));
