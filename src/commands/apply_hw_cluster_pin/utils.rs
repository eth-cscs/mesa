...
pub fn resolve_hw_description_to_xnames(
    mut target_hsm_node_hw_component_count_vec: Vec<(String, HashMap<String, usize>)>,
...
pub fn calculate_target_hsm_pin(
    ...
    let mut group_target_hsm_node_by_score_hashmap: HashMap<usize, Vec<String>> = HashMap::new();
    ...
    let mut group_parent_hsm_node_by_score_hashmap: HashMap<usize, Vec<String>> = HashMap::new();
    ...
    while work_to_do {
        ...
        parent_hsm_node_score_tuple_vec.retain(|(node, _)| !node.eq(&best_candidate.0));
        ...
        (best_candidate, best_candidate_counters) =
            ...
            .ok_or_else(|| Error::Message("ERROR - No best candidate found.".to_string()))?;
